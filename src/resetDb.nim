...
proc product(year: int, category: int64, img: string, locals: varargs[(int64, string, string)]) =
  ...
  for (lang, name, desc) in locals:
    let nameId = db.insertID(sql"insert into productNameImpl(name) values(?)", name)
    db.exec sql"insert into productName(product, lang, nameId) values(?, ?, ?)", product, lang, nameId
